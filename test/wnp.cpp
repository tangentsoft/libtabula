...
main()
{
	...
	if (!libtabula::WindowsNamedPipeConnection::is_wnp(".")) {
		std::cerr << "Failed to identify Windows named pipe" << std::endl;
	
	}
	else if (libtabula::WindowsNamedPipeConnection::is_wnp("bogus")) {
		std::cerr << "Failed to fail for bogus named pipe" << std::endl;
	}
	else if (libtabula::WindowsNamedPipeConnection::is_wnp(0)) {
