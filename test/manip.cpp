...
template <class T>
static bool
is_quoted(const std::string& s, libtabula::Null<T> orig_str, size_t orig_len)
{
...
explicit_query_quote(T test, size_t len)
{
	...
	libtabula::SQLStream s(0);
	s << libtabula::quote << test;
...
// libtabula::String (formerly ColData) when inserted into Query, but
...
main()
{
	...
	failures += test(libtabula::SQLTypeAdapter(s), len) == false;
	failures += test(libtabula::Null<std::string>(s), len) == false;
	return failures;
}
