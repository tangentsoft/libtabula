...
template <typename T>
static bool
test_equality(const libtabula::String& s, T value)
{
...
test_float_conversion()
{
	...
	if (!test_equality(libtabula::String("123.00"), 123)) return false;
	if (!test_equality(libtabula::String("123."), 123)) return false;
	...
	try {
		if (test_equality(libtabula::String("621.20.0"), 621.2)) {
			std::cerr << "Quasi-FP with two decimal points "
					"converting without error!" << std::endl;
		}
		return false;
	}
	catch (const libtabula::BadConversion&) {
		return true;
	}
}
...
static bool
test_int_conversion(const libtabula::String& s, bool throw_expected)
{
	// Try the conversion
...
test_null()
{
	...
	else if (is_null != libtabula::null) {
		std::cerr << "is_null != libtabula:null!" << std::endl;
		return false;
	}
	else {
		return true;
...
// correctly.
static bool
test_numeric(const libtabula::String& s, int value)
{
	return	test_equality(s, static_cast<signed char>(value)) &&
			...
			test_equality(s, static_cast<libtabula::longlong>(value)) &&
			test_equality(s, static_cast<libtabula::ulonglong>(value)) &&
#endif
...
static bool
test_quote_q(const libtabula::String& s, bool expected)
{
...
static bool
test_string_equality(const libtabula::String& s, std::string value)
{
	if (s == value) {
		if (s != value) {
...
static bool
test_string_equality(const libtabula::String& s1,
		const libtabula::String s2)
{
	if (s1 == s2) {
		return true;
	}
...
// Inverse of above.
static bool
test_string_inequality(const libtabula::String& s1,
		const libtabula::String& s2)
{
	if (s1 != s2) {
		return true;
...
main(int, char* argv[])
{
	try {
		int failures = 0;
		libtabula::String definit;
		libtabula::String empty("");
		libtabula::String zero("0");
		libtabula::String nonzero("42");
		libtabula::String intable1("42.");
		libtabula::String intable2("42.0");
		libtabula::String nonint("42.1");

		failures += test_equality(definit, libtabula::Date()) == false;
		failures += test_equality(definit,
				libtabula::DateTime(0, 0, 0, 0, 0, 0)) == false;
		failures += test_equality(definit, libtabula::Time()) == false;
		...
		failures += test_quote_q(libtabula::String("1", typeid(int)),
				false) == false;
