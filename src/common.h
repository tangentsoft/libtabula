...
#define LIBTABULA_QUERY_THISPTR *this

// Work out major platform-specific stuff here.
#if defined(__WIN32__) || defined(_WIN32)
#	define LIBTABULA_PLATFORM_WINDOWS
...
#			define LIBTABULA_HAVE_LOCALTIME_S
#			define LIBTABULA_HAVE_STD__NOINIT
#		endif
...
#	endif
	...
	#ifdef LIBTABULA_MAKING_DLL
		// When making the DLL, export tagged symbols, so they appear
		// in the import library.
		#define LIBTABULA_EXPORT __declspec(dllexport)
	#elif !defined(LIBTABULA_NO_DLL)
		// We must be _using_ the DLL, so import symbols instead.
		#define LIBTABULA_EXPORT __declspec(dllimport)
	#else
...
// define LIBTABULA_NO_LONG_LONGS.
typedef unsigned long long ulonglong;
typedef long long longlong;
#endif
#endif // !defined(DOXYGEN_IGNORE)

#if !defined(LIBTABULA_NO_UNSIGNED_INT_TYPES)
