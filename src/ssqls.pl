...
#if !defined(LIBTABULA_SSQLS_H)
#define LIBTABULA_SSQLS_H

#include "noexceptions.h"
#include "sql_types.h"

#if !defined(LIBTABULA_SSQLS_COMPATIBLE)
...
#if !defined(LIBTABULA_FP_MIN_DELTA)
#	define LIBTABULA_FP_MIN_DELTA $fp_min_delta
#endif
...
#ifdef LIBTABULA_SSQLS_NO_STATICS
#	define LIBTABULA_SSQLS_CONDITIONAL_STATICS(...)
#else
#	define LIBTABULA_SSQLS_CONDITIONAL_STATICS(...) __VA_ARGS__
#endif

#define LIBTABULA_SSQLS_MAX_MEMBERS $max_data_members
...
inline int sql_cmp($type a, $type b) 
{
	if (fabs(a - b) < LIBTABULA_FP_MIN_DELTA) return 0;
	if (a <  b) return -1;
	return 1;
}
---
...
template <typename T>
inline int sql_cmp(const libtabula::Null<T>& a, const libtabula::Null<T>& b) 
{
	if (a == b) return 0;
	if (a <  b) return -1;
	return 1;
...
#define sql_compare_define(NAME) \\
	bool operator == (const NAME &other) const \\
		{return sql_compare_##NAME<libtabula::sql_dummy>(*this,other) == 0;} \\
	bool operator != (const NAME &other) const \\
		{return sql_compare_##NAME<libtabula::sql_dummy>(*this,other) != 0;} \\
	bool operator > (const NAME &other) const \\
		{return sql_compare_##NAME<libtabula::sql_dummy>(*this,other) >  0;} \\
	...
	int cmp (const NAME &other) const \\
		{return sql_compare_##NAME<libtabula::sql_dummy>(*this,other);} \\
...
foreach my $i (1..$max_data_members) {
	...
	foreach my $j (1..$i) {
		if ($j != $i) {
			$compr .= "    cmp = libtabula::sql_cmp(x.C$j , y.C$j ); \\\n";
			$compr .= "    if (cmp) return cmp; \\\n";
		}

		$compr .= "    return libtabula::sql_cmp(x.C$j , y.C$j );" if $j == $i;
		$parm2 .= "const T$j &p$j";
		$parm2 .= ", " unless $j == $i;
		$define.= "C$j (p$j)";
		$define.= ", " unless $j == $i;
...
#define sql_COMPARE__$i(NAME, $parm1) \\
	template <libtabula::sql_dummy_type dummy> \\
...
$enums 
	...
	template <libtabula::sql_dummy_type dummy> int sql_compare_##NAME(const NAME&, const NAME&);

	struct NAME {
$defs 
	NAME() : table_override_(0) { }
	NAME(const libtabula::Row& row);
	void set(const libtabula::Row &row);
	...
	NAME##_value_list<libtabula::quote_type0> value_list() const {
		return value_list(",", libtabula::quote);}
	NAME##_value_list<libtabula::quote_type0> value_list(const char* d) const {
		return value_list(d, libtabula::quote);}
	template <class Manip>
	NAME##_value_list<Manip> value_list(const char* d, Manip m) const;

	NAME##_field_list<libtabula::do_nothing_type0> field_list() const {
		return field_list(",", libtabula::do_nothing);}
	NAME##_field_list<libtabula::do_nothing_type0> field_list(const char* d) const {
		return field_list(d, libtabula::do_nothing);}
	template <class Manip>
	NAME##_field_list<Manip> field_list(const char* d, Manip m) const;

	NAME##_equal_list<libtabula::quote_type0> equal_list(const char* d = ",",
			const char* c = " = ") const
			{ return equal_list(d, c, libtabula::quote); }
	template <class Manip>
	NAME##_equal_list<Manip> equal_list(const char* d, const char* c, Manip m) const;

	/* cus_data */

	NAME##_cus_value_list<libtabula::quote_type0> value_list($cusparms1) const
			{ return value_list(",", libtabula::quote, $cusparmsv); }
	NAME##_cus_value_list<libtabula::quote_type0> value_list($cusparms2) const
			{ return value_list(",", libtabula::quote, $cusparmsv); }
	NAME##_cus_value_list<libtabula::quote_type0> value_list(std::vector<bool> *i) const
			{ return value_list(",", libtabula::quote, i); }
	NAME##_cus_value_list<libtabula::quote_type0> value_list(libtabula::sql_cmp_type sc) const
			{ return value_list(",", libtabula::quote, sc); }

	NAME##_cus_value_list<libtabula::quote_type0> value_list(const char* d, $cusparms1) const
			{ return value_list(d, libtabula::quote, $cusparmsv); }
	NAME##_cus_value_list<libtabula::quote_type0> value_list(const char* d, $cusparms2) const
			{ return value_list(d, libtabula::quote, $cusparmsv); }
	NAME##_cus_value_list<libtabula::quote_type0> value_list(const char* d,
			std::vector<bool> *i) const
			{ return value_list(d, libtabula::quote, i); }
	NAME##_cus_value_list<libtabula::quote_type0> value_list(const char* d,
			libtabula::sql_cmp_type sc) const
			{ return value_list(d, libtabula::quote, sc); }
	...
	NAME##_cus_field_list<libtabula::do_nothing_type0> field_list($cusparms1) const 
			{ return field_list(",", libtabula::do_nothing, $cusparmsv); }
	NAME##_cus_field_list<libtabula::do_nothing_type0> field_list($cusparms2) const
			{ return field_list(",", libtabula::do_nothing, $cusparmsv); }
	NAME##_cus_field_list<libtabula::do_nothing_type0> field_list(std::vector<bool> *i) const
			{ return field_list(",", libtabula::do_nothing, i); }
	NAME##_cus_field_list<libtabula::do_nothing_type0> field_list(libtabula::sql_cmp_type sc) const
			{ return field_list(",", libtabula::do_nothing, sc); }

	NAME##_cus_field_list<libtabula::do_nothing_type0> field_list(const char* d,
			$cusparms1) const
			{ return field_list(d, libtabula::do_nothing, $cusparmsv); }
	NAME##_cus_field_list<libtabula::do_nothing_type0> field_list(const char* d,
			$cusparms2) const
			{ return field_list(d, libtabula::do_nothing, $cusparmsv); }
	NAME##_cus_field_list<libtabula::do_nothing_type0> field_list(const char* d,
			std::vector<bool>* i) const
			{ return field_list(d, libtabula::do_nothing, i); }
	NAME##_cus_field_list<libtabula::do_nothing_type0> field_list(const char* d,
			libtabula::sql_cmp_type sc) const
			{ return field_list(d, libtabula::do_nothing, sc); }
	...
	NAME##_cus_equal_list<libtabula::quote_type0> equal_list($cusparms1) const
			{ return equal_list(",", " = ", libtabula::quote, $cusparmsv); }
	NAME##_cus_equal_list<libtabula::quote_type0> equal_list($cusparms2) const
			{ return equal_list(",", " = ", libtabula::quote, $cusparmsv); }
	NAME##_cus_equal_list<libtabula::quote_type0> equal_list(std::vector<bool>* i) const
			{ return equal_list(",", " = ", libtabula::quote, i); }
	NAME##_cus_equal_list<libtabula::quote_type0> equal_list(libtabula::sql_cmp_type sc) const
			{ return equal_list(",", " = ", libtabula::quote, sc); }

	NAME##_cus_equal_list<libtabula::quote_type0> equal_list(const char* d, $cusparms1) const
			{ return equal_list(d, " = ", libtabula::quote, $cusparmsv); }
	NAME##_cus_equal_list<libtabula::quote_type0> equal_list(const char* d, $cusparms2) const
			{ return equal_list(d, " = ", libtabula::quote, $cusparmsv); }
	NAME##_cus_equal_list<libtabula::quote_type0> equal_list(const char* d,
			std::vector<bool> *i) const
			{ return equal_list(d, " = ", libtabula::quote, i); }
	NAME##_cus_equal_list<libtabula::quote_type0> equal_list(const char* d,
			libtabula::sql_cmp_type sc) const
			{ return equal_list(d, " = ", libtabula::quote, sc); }

	NAME##_cus_equal_list<libtabula::quote_type0> equal_list(const char* d, const char* c,
			$cusparms1) const
			{ return equal_list(d, c, libtabula::quote, $cusparmsv); }
	NAME##_cus_equal_list<libtabula::quote_type0> equal_list(const char* d, const char* c,
			$cusparms2) const
			{ return equal_list(d, c, libtabula::quote, $cusparmsv); }
	NAME##_cus_equal_list<libtabula::quote_type0> equal_list(const char* d, const char* c,
			std::vector<bool> *i) const
			{ return equal_list(d, c, libtabula::quote, i); }
	NAME##_cus_equal_list<libtabula::quote_type0> equal_list(const char* d, const char* c,
			libtabula::sql_cmp_type sc) const
			{ return equal_list(d, c, libtabula::quote, sc); }
	...
	LIBTABULA_SSQLS_CONDITIONAL_STATICS(
...
$cus_equal_list
	...
	NAME::value_list(const char* d, Manip m, libtabula::sql_cmp_type /*sc*/) const
			{ sql_compare_type_def_##CMP(NAME, value, NUM); }

	template <class Manip>
	inline NAME##_cus_field_list<Manip> 
	NAME::field_list(const char* d, Manip m, libtabula::sql_cmp_type /*sc*/) const
			{ sql_compare_type_def_##CMP(NAME, field, NUM); }

	template <class Manip>
	inline NAME##_cus_equal_list<Manip> 
	NAME::equal_list(const char* d, const char* c, Manip m, libtabula::sql_cmp_type /*sc*/) const
			{ sql_compare_type_defe_##CMP(NAME, equal, NUM); }

	template <libtabula::sql_dummy_type dummy>
	void populate_##NAME(NAME *s, const libtabula::Row &row)
	{
		libtabula::NoExceptions ignore_schema_mismatches(row);
$popul
	}

	inline NAME::NAME(const libtabula::Row& row) :
	table_override_(0)
			{ populate_##NAME<libtabula::sql_dummy>(this, row); }
	inline void NAME::set(const libtabula::Row& row)
	{
		table_override_ = 0;
		populate_##NAME<libtabula::sql_dummy>(this, row);
	}
...
#endif // !defined(LIBTABULA_SSQLS_H)
