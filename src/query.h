...
#if !defined(LIBTABULA_QUERY_H)
#define LIBTABULA_QUERY_H
...
/// \link libtabula::Query::execute() exec*(), \endlink
/// \link libtabula::Query::store() store*(), \endlink or use() methods.
...
/// \link libtabula::Query::execute() exec*(), \endlink
/// \link libtabula::Query::store() store*(), \endlink or
/// \link libtabula::Query::use() use() \endlink takes no parameters,
...
/// \link libtabula::Query::insert() insert() \endlink method, which
...
/// the many
/// \link libtabula::Query::execute(const SQLTypeAdapter&) exec*(), \endlink
/// \link libtabula::Query::store(const SQLTypeAdapter&) store*(), \endlink
/// or \link libtabula::Query::use(const SQLTypeAdapter&) use() \endlink
...
public:
	...
	///
	/// This is the constructor used by libtabula::Connection::query().
	///
	...
	{	
		libtabula::UseQueryResult res = use(query);
		if (res) {
			libtabula::NoExceptions ne(res);
	...
	{	
		libtabula::UseQueryResult res = use();
		if (res) {
			libtabula::NoExceptions ne(res);
			while (libtabula::Row row = res.fetch_row()) {
				fn(row);
	...
	/// One uses it with libtabula's Specialized SQL Structures mechanism.
	///
	/// \param o old row
	/// \param n new row
	///
	...
	Query& update(const T& o, const T& n)
	{
		...
		LIBTABULA_QUERY_THISPTR << std::setprecision(16) <<
	...
	/// libtabula's Specialized SQL Structures mechanism.
	///
	/// \param v new row
	///
	/// \sa replace(), update()
	...
	Query& insert(Iter first, Iter last)
	{
		...
		if (first != last) {
			...
			Iter it = first;
			while (++it != last) {
				LIBTABULA_QUERY_THISPTR << ",(" << it->value_list() << ')';
			}
		}
...
#endif // !defined(LIBTABULA_QUERY_H)
