...
#if !defined(LIBTABULA_MYSTRING_H)
#define LIBTABULA_MYSTRING_H
...
class LIBTABULA_EXPORT SQLTypeAdapter;
...
/// defining LIBTABULA_NO_BINARY_OPERS.
///
...
#if defined(LIBTABULA_PLATFORM_VISUAL_CPP)
...
#   pragma warning(default: 4244)
	...
	///
	/// \throw libtabula::BadIndex if the string is not initialized or there
	...
	/// \brief Converts this object's string data to a libtabula::Date
	operator Date() const { return buffer_ ? Date(*this) : Date(); }

	/// \brief Converts this object's string data to a libtabula::DateTime
	operator DateTime() const
			{ return buffer_ ? DateTime(*this) : DateTime(); }

	/// \brief Converts this object's string data to a libtabula::Time
...
#if !defined(LIBTABULA_NO_BINARY_OPERS) && !defined(DOXYGEN_IGNORE)
// Ignore this section is LIBTABULA_NO_BINARY_OPERS is defined, or if this
...
#endif // !defined(LIBTABULA_NO_BINARY_OPERS) && !defined(DOXYGEN_IGNORE)


#if !defined(DOXYGEN_IGNORE)
// Doxygen isn't smart enough to recognize these template
// specializations.  Maybe it's the LIBTABULA_EXPORT tags?
...
template <> LIBTABULA_EXPORT bool String::conv(bool) const;
...
/// breaks into.
template <> LIBTABULA_EXPORT String String::conv(String) const;

/// \brief Specialization of String::conv<Type>() for C++ strings
template <> LIBTABULA_EXPORT std::string String::conv(std::string) const;
...
#endif // !defined(LIBTABULA_MYSTRING_H)
