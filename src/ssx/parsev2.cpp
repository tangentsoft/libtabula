		...
		const File& file)
{
	...
	while (tl.end() - it > 1) {
		...
		libtabula::internal::str_to_lwr(attr);
...
value_(ft_string)
{
	...
	string ls(s);
	libtabula::internal::str_to_lwr(ls);
...
ParseV2::File::split_path(StringList& parts, const std::string& path) const
{
	...
	const char sep = LIBTABULA_PATH_SEPARATOR;
		...
		const File& file)
{
	...
	libtabula::internal::str_to_lwr(directive);
...
ParseV2::Option::parse_bool(const std::string& value)
{
	...
	string b(value);
	libtabula::internal::str_to_lwr(b);
