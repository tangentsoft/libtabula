...
// returns true.
static bool
is_jpeg(const libtabula::sql_blob& img, const char** whynot)
{
...
static bool
load_jpeg_file(const libtabula::examples::CommandLine& cmdline,
		images& img, string& img_name)
{
	...
	if (img_file) {
		...
		// and assigning that directly to a libtabula::sql_blob, we avoid
...
main(int argc, char *argv[])
{
	...
	try {
		...
		images img(libtabula::null, libtabula::null);
		...
		if (load_jpeg_file(cmdline, img, img_name)) {
			...
			// binary data in a libtabula::sql_blob, which avoids data
			...
			// demonstrating that libtabula::Null<libtabula::sql_blob>
			// is legal in SSQLS.
