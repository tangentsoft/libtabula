/***********************************************************************
 ...
 Specialized SQL Structures feature of libtabula and Query::insertfrom().

 Copyright © 1998 by Kevin Atkinson, © 1999-2001 by MySQL AB,
 © 2004-2009 by Educational Technology Resources, Inc., © 2008 by 
...
// strings.
static size_t
tokenize_line(const string& line, vector<libtabula::String>& strings)
{
	string field;
	strings.clear();
	...
	while (getline(iss, field, '\t')) {
		strings.push_back(libtabula::String(field));
	}
...
read_stock_items(const char* filename, vector<stock>& stock_vector)
{
	...
	string line;
	vector<libtabula::String> strings;
...
main(int argc, char *argv[])
{
	...
	try {
		...
		libtabula::Query::MaxPacketInsertPolicy<> insert_policy(1000);
	...
	catch (const libtabula::BadInsertPolicy& er) {
		// Handle bad conversions
