...
main(int argc, char *argv[])
{
	...
	if (libtabula::get_library_version() != LIBTABULA_HEADER_VERSION) {
		cerr << "Version mismatch: library is v" <<
				version_str(libtabula::get_library_version()) <<
				", headers are v" <<
				version_str(LIBTABULA_HEADER_VERSION) <<
	...
	bool new_db = false;
	{
		libtabula::NoExceptions ne(con);
		libtabula::Query query = con.query();
		if (con.select_db(libtabula::examples::db_name)) {
			...
			if (con.create_db(libtabula::examples::db_name) &&
					con.select_db(libtabula::examples::db_name)) {
				new_db = true;
			}
			else {
	...
	try {
		...
		query.template_defaults["desc"] = libtabula::null;
